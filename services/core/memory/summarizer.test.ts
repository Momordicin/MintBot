...
// 常量完全一致的值（60 / 50），保证本文件已有的真值表断言继续成立。summarizer.ts 不再引用
// getBackgroundModelProviderConfig（maxTokens 现在随 deps.model 一起从 index.ts 组装时带入，
// 见 ModelProvider.resolveMaxTokens），本文件因此只需要 mock getMemoryConfig 这一个函数
